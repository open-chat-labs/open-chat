...
impl AlertId {
    pub fn try_parse(ext_id: &str) -> Option<AlertId> {
        let mut ext_id = ext_id.to_owned();
        if ext_id.starts_with("in_") {
            ext_id.replace_range(0..3, "");
            ext_id.parse::<u32>().map(AlertId::Internal).ok()
        } else if ext_id.starts_with("gd_") {
            ext_id.replace_range(0..3, "");
            Principal::from_text(&ext_id).map(|id| AlertId::GroupDeleted(id.into())).ok()
        } else {
            None
        }
    }
...
impl Display for AlertId {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let ext_id = match self {
            ...
            AlertId::GroupDeleted(chat_id) => format!("gd_{}", Principal::from(*chat_id).to_text()),
        };
