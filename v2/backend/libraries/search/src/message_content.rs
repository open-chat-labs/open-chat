    ...
    fn from(message_content: &MessageContent) -> Self {
        ...
        match message_content {
            ...
            MessageContent::Cycles(c) => {
                ...
                document.add_field(format_as_whole_units(c.amount as f64, TRILLION as f64), 0.1);
                try_add_caption(&mut document, c.caption.as_ref())
            }
            MessageContent::ICP(c) => {
                document.add_field("icp".to_owned(), 0.1);
                document.add_field(format_as_whole_units(c.amount_e8s as f64, E8S_PER_ICP as f64), 0.1);
                try_add_caption(&mut document, c.caption.as_ref())
            }
