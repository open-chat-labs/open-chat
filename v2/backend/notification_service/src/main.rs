...
async fn my_handler(request: Request, _: Context) -> Result<(), Error> {
    match request.run_mode {
        Mode::PushNotifications => actions::push_notifications::run(request.canister_id).await,
        Mode::PruneNotifications => actions::prune_notifications::run(request.canister_id).await,
    }
}
