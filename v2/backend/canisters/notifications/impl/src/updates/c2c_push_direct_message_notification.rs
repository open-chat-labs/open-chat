...
fn c2c_push_direct_message_notification(args: Args) -> Response {
    RUNTIME_STATE.with(|state| c2c_push_direct_message_notification_impl(args, state.borrow_mut().as_mut().unwrap()))
}

fn c2c_push_direct_message_notification_impl(args: Args, runtime_state: &mut RuntimeState) -> Response {
        ...
        .contains_any(&[args.recipient], MAX_SUBSCRIPTION_AGE, now)
    {
        runtime_state.data.notifications.add(NotificationEnvelope {
            ...
            notification: Notification::DirectMessageNotification(args.notification),
        });
    }
