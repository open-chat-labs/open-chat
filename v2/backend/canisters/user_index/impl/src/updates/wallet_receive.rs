...
const THREE_TRILLION: u128 = 3_000_000_000_000;

#[update]
#[trace]
fn wallet_receive() {
    let cycles_available = ic_cdk::api::call::msg_cycles_available() as Cycles;
    if cycles_available < THREE_TRILLION {
        // If the cycles amount is < 3T, we assume that the payment is being made to register a user
        // in which case we either successfully register a user or we refund the cycles.
        mutate_state(|state| try_confirm_user(cycles_available, state));
    } else {
        // If the cycles amount is >= 3T, we assume this is a donation / top-up and accept all of
        // the cycles
        accept_cycles();
    }
}
