...
fn c2c_retry_sending_failed_messages_impl(args: Args, runtime_state: &mut RuntimeState) -> Response {
    ...
    if !messages_to_retry.is_empty() {
        ...
        ic_cdk::block_on(retry_sending_messages(my_user_id, args.recipient, messages_to_retry));
    }
    Success
}
...
async fn retry_sending_messages(
    ...
    let futures: Vec<_> = messages_to_retry
        .into_iter()
        .map(|(args, cycles_transfer)| send_to_recipients_canister(my_user_id, recipient, args, cycles_transfer, true))
        .collect();
