...
export class CachingUserClient implements IUserClient {
    ...
    constructor(private client: IUserClient) {
    ...
    private async getCachedMessages(
        ...
        const cachedMsgs = await db.getAll(
            "chat_messages",
            IDBKeyRange.bound(createKey(userId, fromIndex), createKey(userId, toIndex))
        );
    ...
    async chatMessages(
        ...
        const cachedMsgs = await this.getCachedMessages(userId, fromIndex, toIndex);
        ...
        const resp = await this.client.chatMessages(userId, fromIndex, toIndex);
        if (resp !== "chat_not_found") {
            ...
            msgs.forEach(async (msg) => {
                await store.put(msg, createKey(userId, msg.messageIndex));
            });
            await tx.done;
        }
        return resp;
