...
fn remove_member_from_channel_succeeds() {
    ...
    let user2_summary1 = client::community::happy_path::summary(env, &user2, community_id);
    ...
    let user2_summary2 = client::community::happy_path::summary(env, &user2, community_id);
    assert!(!user2_summary2.channels.iter().any(|c| c.channel_id == channel_id));

    // Check that the channel is still returned for user1
    let user1_summary = client::community::happy_path::summary(env, &user1, community_id);
