...
- Give each user their own copy of a direct chat, as in the User canister, rather than the two users sharing one copy of its events, and deliver messages and read receipts between the copies of two users in the same canister directly ([#TODO](https://github.com/open-chat-labs/open-chat/pull/TODO))
- Drop messages from a user the recipient has blocked on the recipient's side, as between User canisters, so a chat deleted by one user is unaffected on the other's side and comes back as a fresh copy when they are messaged again ([#TODO](https://github.com/open-chat-labs/open-chat/pull/TODO))
- Garbage collect a deleted chat's stable memory entries within the scope of the user who deleted it, and remove the `direct_chat_cores` metric ([#TODO](https://github.com/open-chat-labs/open-chat/pull/TODO))
