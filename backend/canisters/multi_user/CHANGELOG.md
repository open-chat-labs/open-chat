...
- Drop messages from a user the recipient has blocked on the recipient's side, as between User canisters, so a chat deleted by one user is unaffected on the other's side and comes back as a fresh copy when they are messaged again ([#9415](https://github.com/open-chat-labs/open-chat/pull/9415))
