...
// Reads the events of `user_id`'s chat with `them`, provided the caller may act as `user_id`.
// This is the User canister's `read_events` with the chat reached via the user rather than held
// directly by the canister.
pub(crate) fn read_events<A, F: FnOnce(A, UserId, ChatEventsListReader) -> Vec<EventOrExpiredRange>>(
    ...
    if let Err(now) = check_replica_up_to_date(latest_known_update, state) {
        return Error(OCErrorCode::ReplicaNotUpToDate.with_message(now));
    }

    let user_index = match state.authorized_user_index(user_id) {
        Ok(index) => index,
