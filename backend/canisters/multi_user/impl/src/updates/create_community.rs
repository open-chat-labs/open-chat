    ...
    .await
    {
        Ok(response) => match response {
            c2c_create_community::Response::Success(r) => {
                mutate_state(|state| commit(prepare_result.my_index, r.community_id, r.local_user_index_canister_id, state));
...
fn prepare(args: Args, state: &RuntimeState) -> OCResult<PrepareResult> {
    state.with_caller_user(|my_index, user| prepare_for_user(args, my_index, user, state))
}

fn prepare_for_user(args: Args, my_index: u16, user: &User, state: &RuntimeState) -> OCResult<PrepareResult> {
        ...
        .unwrap_or_default()
    {
        ...
        let create_community_args = c2c_create_community::Args {
            ...
            user_id: Some(state.user_id(my_index)),
        };
        Ok(PrepareResult {
            my_index,
