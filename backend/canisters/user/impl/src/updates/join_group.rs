    ...
    match group_canister_c2c_client::c2c_join_group_v2(args.chat_id.into(), &c2c_args).await {
        Ok(result) => match result {
            c2c_join_group::Response::Success(summary) => {
                ...
                mutate_state(|state| commit(args.chat_id, args.as_super_admin, summary.notifications_muted ,latest_message_index, state));
...
fn commit(chat_id: ChatId, as_super_admin: bool, notifications_muted: bool, latest_message_index: Option<MessageIndex>, runtime_state: &mut RuntimeState) {
