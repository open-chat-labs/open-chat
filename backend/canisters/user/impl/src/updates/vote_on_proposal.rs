...
use types::{CanisterId, ChatId, MessageIndex, NeuronId, ProposalId};
...
async fn vote_on_proposal(args: Args) -> Response {
    ...
    if !voted.is_empty() && errors.is_empty() {
        ic_cdk::spawn(record_vote_in_group(args.chat_id, args.message_index, args.adopt));
    }
...
async fn record_vote_in_group(chat_id: ChatId, message_index: MessageIndex, adopt: bool) {
    let c2c_args = group_canister::c2c_record_proposal_vote::Args { message_index, adopt };
    let _ = group_canister_c2c_client::c2c_record_proposal_vote(chat_id.into(), &c2c_args).await;
}
