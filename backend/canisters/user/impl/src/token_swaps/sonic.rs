...
impl SwapClient for SonicClient {
    ...
    async fn deposit(&self, amount: u128) -> Result<u128, C2CError> {
        ...
        match sonic_canister_c2c_client::deposit(self.sonic_canister_id, args).await?.0 {
            ...
            SonicResult::Err(error) => Err(convert_error(self.sonic_canister_id, "deposit", error)),
        }
    }
...
pub async fn withdraw(swap_canister_id: CanisterId, ledger_canister_id: CanisterId, amount: u128) -> Result<u128, C2CError> {
    ...
    match sonic_canister_c2c_client::withdraw(swap_canister_id, args).await?.0 {
        ...
        SonicResult::Err(error) => Err(convert_error(swap_canister_id, "withdraw", error)),
    }
}

async fn retrieve_subaccount(sonic_canister_id: CanisterId) -> Result<[u8; 32], C2CError> {
