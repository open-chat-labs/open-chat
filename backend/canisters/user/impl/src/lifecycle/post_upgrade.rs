...
fn post_upgrade(args: Args) {
    ...
    mutate_state(|state| {
        ...
        for chat in state.data.direct_chats.iter_mut() {
            let video_call = chat.events.video_call_in_progress();
            if video_call.timestamp < now.saturating_sub(HOUR_IN_MS) {
                if let Some(message_id) = video_call.as_ref().map(|vc| vc.message_index).and_then(|index| {
                    chat.events
                        .main_events_reader()
