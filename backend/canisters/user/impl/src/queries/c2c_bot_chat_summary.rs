...
fn c2c_bot_chat_summary_impl(args: Args, state: &RuntimeState) -> OCResult<ChatSummaryDirect> {
    ...
    let events_ttl = chat.core.events.get_events_time_to_live();
    let main_events_reader = chat.core.events.visible_main_events_reader(EventIndex::default());
    ...
    Ok(ChatSummaryDirect {
        ...
        video_call_in_progress: chat.core.events.video_call_in_progress(None),
    })
}
