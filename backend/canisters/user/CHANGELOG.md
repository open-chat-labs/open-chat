...
- Collapse `DirectChat` back into a single struct holding its events and both read positions, in the `direct_chat` library (renamed from `direct_chat_core`), while still reading chats serialized in the split shape, and align both read positions of the user's chat with themselves on reading it ([#9415](https://github.com/open-chat-labs/open-chat/pull/9415))
