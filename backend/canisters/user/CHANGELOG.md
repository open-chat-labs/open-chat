...
- Move each chat's `MessageId` to `EventIndex` map from the heap into the stable memory map for small entries ([#PRNUM](https://github.com/open-chat-labs/open-chat/pull/PRNUM))
