...
- Add `c2c_local_user_index_v2`, taking events which each name the user they are for, since a batch sent to a MultiUser canister can hold the events of any of its users. This canister holds a single user, so it applies them all to that user ([#9442](https://github.com/open-chat-labs/open-chat/pull/9442))
...
- Move the referrals model and the `Membership` enum into the shared `user_state` library, so the MultiUser canister can hold them per user ([#9442](https://github.com/open-chat-labs/open-chat/pull/9442))
...
- A few fixes for the OneSec integration ([#8507](https://github.com/open-chat-labs/open-chat/pull/8507))
...
- Update BTC balance via User canister and send message once updated ([#7611](https://github.com/open-chat-labs/open-chat/pull/7611))
...
- Change direct chat summary so `latest_message` is optional ([#7542](https://github.com/open-chat-labs/open-chat/pull/7542))
