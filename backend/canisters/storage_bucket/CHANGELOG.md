...
- Optional `source_hash` on `upload_chunk_v2`: the hash of the bytes a client transcoded from (video re-encoded at upload). Checked against the CSAM denylist and vault pins alongside the stored hash, and denylisted with it when a verdict lands, so re-uploads of an original whose transcode was upheld as CSAM are still refused ([#9254](https://github.com/open-chat-labs/open-chat/pull/9254))
