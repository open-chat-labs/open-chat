...
- Add `migrate_users`, for platform operators, which queues users to be migrated to MultiUser canisters, and `set_user_migration_concurrency`. Each queued user, suspended or not, is assigned the MultiUser canister with the fewest users (or, in tests, the canister given) and sent to their LocalUserIndex in a `StartUserMigration` event, and the LocalUserIndex reports back whether the migration started. Add the `user_migration` query for platform operators, which returns a user's migration status. The LocalUserIndexes must be upgraded first ([#9582](https://github.com/open-chat-labs/open-chat/pull/9582))
