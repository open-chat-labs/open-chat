...
pub(crate) fn start_job_if_required(state: &RuntimeState) -> bool {
    if TIMER_ID.get().is_none() && !state.data.user_principal_updates_queue.is_empty() {
        ...
        trace!("'user_principal_updates_queue' job started");
        true
    } else {
        false
    }
...
pub fn run() {
    ...
    if !next_batch.is_empty() {
        ...
        trace!("'user_principal_updates_queue' job stopped");
    }
}
...
async fn notify(canister_id: CanisterId, args: UpdateUserPrincipalArgs) {
    ...
    mutate_state(|state| match response {
        ...
        Err((RejectionCode::DestinationInvalid, _)) => state.data.user_principal_updates_queue.mark_success(args.user_id),
        Err(_) => {
