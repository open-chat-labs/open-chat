...
use user_index_canister::start_user_migration::{Response::*, *};

// Starts migrating a user in a canister of their own to a MultiUser canister, as the UserIndex will
// once it migrates users itself, so that the flow can be tested in the meantime. Only available in
// test mode.
#[update(guard = "caller_is_governance_principal", msgpack = true)]
#[trace]
async fn start_user_migration(args: Args) -> Response {
    ...
    match user_canister_c2c_client::c2c_try_start_migration(
        ...
        &user_canister::c2c_try_start_migration::Args {
