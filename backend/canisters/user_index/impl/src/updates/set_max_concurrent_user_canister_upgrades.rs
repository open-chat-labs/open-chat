...
use local_user_index_canister::c2c_notify_user_index_events::{LocalUserIndexEvent, MaxConcurrentCanisterUpgradesChanged};
use tracing::info;
...
fn set_max_concurrent_user_canister_upgrades_impl(args: Args, runtime_state: &mut RuntimeState) -> Response {
    runtime_state.data.push_event_to_all_local_user_indexes(
        LocalUserIndexEvent::MaxConcurrentCanisterUpgradesChanged(MaxConcurrentCanisterUpgradesChanged { value: args.value }),
        None,
    );
