...
use local_user_index_canister::report_message::{Response::*, *};
...
async fn report_message(args: Args) -> Response {
    ...
    let c2c_args = group_canister::c2c_report_message::Args {
        user_id,
    ...
    match group_canister_c2c_client::c2c_report_message(platform_moderators_group.into(), &c2c_args).await {
        Ok(_) => Success,
        Err(error) => InternalError(format!("Failed to call 'group::c2c_report_message': {error:?}")),
    }
}
