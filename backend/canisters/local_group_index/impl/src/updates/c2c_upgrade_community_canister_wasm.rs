...
fn c2c_upgrade_community_canister_wasm_impl(args: Args, runtime_state: &mut RuntimeState) -> Response {
    let version = args.wasm.version;

    if !runtime_state.data.test_mode && version < runtime_state.data.community_canister_wasm_for_new_canisters.version {
        VersionNotHigher
    } else {
        runtime_state.data.communities_requiring_upgrade.clear();
        if args.use_for_new_canisters.unwrap_or(true) {
            runtime_state.data.community_canister_wasm_for_new_canisters = args.wasm.clone();
        }
        runtime_state.data.community_canister_wasm_for_upgrades = args.wasm;
        ...
        {
            runtime_state.data.communities_requiring_upgrade.enqueue(canister_id)
        }

        let canisters_queued_for_upgrade = runtime_state.data.communities_requiring_upgrade.count_pending();
