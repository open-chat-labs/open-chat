...
fn prepare(args: Args, runtime_state: &mut RuntimeState) -> Result<PrepareOk, Response> {
    ...
    let canister_wasm = runtime_state.data.community_canister_wasm_for_new_canisters.clone();
...
fn commit(community_id: CommunityId, wasm_version: Version, runtime_state: &mut RuntimeState) {
    runtime_state.data.local_communities.add(community_id, wasm_version);
}
