...
pub async fn upload_wasm_in_chunks(agent: &Agent, canister_id: &CanisterId, wasm: &[u8], canister_type: ChildCanisterType) {
    for (index, chunk) in wasm.chunks(1_000_000).enumerate() {
        ...
        assert!(matches!(response, upload_wasm_chunk::Response::Success(_)));
    }
}
