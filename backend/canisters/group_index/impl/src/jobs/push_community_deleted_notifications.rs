...
pub(crate) fn start_job_if_required(runtime_state: &RuntimeState) -> bool {
    if TIMER_ID.with(|t| t.get().is_none()) && runtime_state.data.deleted_communities.notifications_pending() > 0 {
...
fn next_batch(runtime_state: &mut RuntimeState) -> Option<Vec<(UserId, DeletedCommunityInfo)>> {
    if runtime_state.data.deleted_communities.notifications_pending() == 0 {
        None
    } else {
        Some(
            (0..MAX_BATCH_SIZE)
                .map_while(|_| {
                    runtime_state
                        .data
                        .deleted_communities
                        .dequeue_community_deleted_notification()
                })
                .collect(),
        )
    }
