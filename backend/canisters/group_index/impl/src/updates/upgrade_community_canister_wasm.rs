...
async fn upgrade_community_canister_wasm(args: Args) -> Response {
    ...
    let futures: Vec<_> = local_index_canisters
        .into_iter()
        .map(|(canister_id, filter)| upgrade_community_wasm_in_local_index(canister_id, &wasm, wasm_hash, Some(filter)))
        .collect();
...
pub(crate) async fn upgrade_community_wasm_in_local_index(
    ...
    let push_wasm_response = local_user_index_canister_c2c_client::push_wasm_in_chunks(
        canister_id,
        local_user_index_canister::ChildCanisterType::Community,
    ...
    let upgrade_response = local_user_index_canister_c2c_client::c2c_upgrade_community_canister_wasm(
        canister_id,
        &local_user_index_canister::c2c_upgrade_community_canister_wasm::Args {
