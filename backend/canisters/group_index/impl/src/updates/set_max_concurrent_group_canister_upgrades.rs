...
async fn set_max_concurrent_group_canister_upgrades(args: Args) -> Response {
    ...
    let args = local_user_index_canister::c2c_set_max_concurrent_group_upgrades::Args { value: args.value };

    let futures: Vec<_> = canisters
        .into_iter()
        .map(|canister_id| local_user_index_canister_c2c_client::c2c_set_max_concurrent_group_upgrades(canister_id, &args))
        .collect();
