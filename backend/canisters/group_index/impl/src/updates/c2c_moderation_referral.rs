...
use group_index_canister::c2c_moderation_referral::*;
use types::Chat;

#[update(guard = "caller_is_group_or_community_canister", msgpack = true)]
#[trace]
fn c2c_moderation_referral(args: Args) -> Response {
    read_state(|state| c2c_moderation_referral_impl(args, state))
}

fn c2c_moderation_referral_impl(args: Args, state: &RuntimeState) -> Response {
    ...
    state.data.fire_and_forget_handler.send(
        ...
        msgpack::serialize_then_unwrap(user_index_canister::c2c_moderation_referral::Args {
            chat_id,
