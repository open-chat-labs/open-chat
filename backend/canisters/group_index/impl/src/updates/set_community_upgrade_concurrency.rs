...
async fn set_community_upgrade_concurrency(args: Args) -> Response {
    ...
    let args = local_user_index_canister::c2c_set_community_upgrade_concurrency::Args { value: args.value };

    let futures: Vec<_> = local_index_canisters
        .into_iter()
        .map(|canister_id| local_user_index_canister_c2c_client::c2c_set_community_upgrade_concurrency(canister_id, &args))
        .collect();
