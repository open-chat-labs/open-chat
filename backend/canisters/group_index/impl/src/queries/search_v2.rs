...
use group_index_canister::search_v2::{Response::*, *};
...
#[query]
fn search_v2(args: Args) -> Response {
...
fn search_impl(args: Args, state: &RuntimeState) -> Response {
    ...
    let (search_groups, search_communities) = match args.scope {
        Scope::All => (true, true),
        Scope::Communities => (false, true),
        Scope::Groups => (true, false),
    };
...
mod tests {
    ...
    fn max_results_respected() {
        ...
        if let Response::Success(result) = response {
            assert_eq!(2, result.group_matches.len());
        } else {
            panic!();
        }
    }
    ...
    fn results_in_expected_order() {
        ...
        if let Response::Success(result) = response {
            assert_eq!(6, result.group_matches.len());
            ...
            for (i, expected_group) in expected_groups.into_iter().enumerate() {
                ...
                assert_eq!(result.group_matches[i].chat_id, chat_id);
            }
        } else {
            panic!();
        }
