...
- Reject every update call while the community is frozen in one place, `execute_update`, by trapping, rather than per endpoint with a `CommunityFrozen` error. The endpoints which must keep working while frozen (unfreezing, platform moderation, and those which didn't check before) run via `execute_update_even_if_frozen` ([#9533](https://github.com/open-chat-labs/open-chat/pull/9533))
