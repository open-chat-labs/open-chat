...
// Sends a message to a channel holding a transfer the caller makes from their own funds: crypto
// sent to another member, a prize, or a P2P swap. The community never pays from its own account,
// so the transfer of a crypto or prize message must be either:
...
// wallet.
#[ts_export(community, send_message_with_transfer)]
