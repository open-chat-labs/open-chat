...
// its MessageIndex -> EventIndex map, its expiring events, its events' last updated timestamps,
// its users' metrics and its search index) into stable memory. It is also used to move the users'
// metrics of imported groups into stable memory (see `finalize_group_import`), so it must be kept
// for that even once every channel has been migrated.
