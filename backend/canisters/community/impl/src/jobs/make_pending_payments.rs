...
use types::{CanisterId, Cryptocurrency, TimestampNanos, UserId};
        ...
        Ok(_block_index) => {
            if matches!(pending_payment.reason, PendingPaymentReason::AccessGate) {
                if let PaymentRecipient::Member(user_id) = pending_payment.recipient {
                    mutate_state(|state| {
                        record_access_gate_payment(user_id, pending_payment.ledger_canister, pending_payment.amount, state);
                    });
                }
            }
        }
...
fn record_access_gate_payment(user_id: UserId, ledger_canister: CanisterId, amount: u128, state: &mut RuntimeState) {
    state.data.total_payment_receipts.add(ledger_canister, amount);

    if let Some(member) = state.data.members.get_by_user_id_mut(&user_id) {
        member.payment_receipts.add(ledger_canister, amount);
    }
}
