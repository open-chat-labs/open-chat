...
//
// Joining a payment gated community or channel makes an async call to transfer tokens from the user
// partway through the join. If a user triggers the join twice in quick succession, both calls can
// take the payment before either has recorded the user as a member, resulting in the user being
// charged multiple times but only joining once. Acquiring the lock before taking the payment
// prevents this.
#[derive(Default)]
