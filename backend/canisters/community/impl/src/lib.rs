...
impl Data {
    ...
    // looked up by their latest id. `previous_user_ids` must be ordered oldest first. If anything was
    // held under a previous id, the migrations are also cached, since events may refer to the user by
    // their previous ids. Returns whether anything was moved.
    ...
    fn migrate_user_id(&mut self, old_user_id: UserId, new_user_id: UserId, now: TimestampMillis) -> bool {
        ...
        let mut migrated = self.members.migrate_user_id(old_user_id, new_user_id, now);
