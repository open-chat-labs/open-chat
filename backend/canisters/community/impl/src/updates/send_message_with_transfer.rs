...
use crate::updates::send_message::{prepare as prepare_to_send, send_message_with_completed_transfer};
...
use community_canister::send_message_with_transfer::{Response::*, *};
...
fn prepare(args: &Args, state: &mut RuntimeState) -> OCResult<(UserId, PrepareResult)> {
    ...
    prepare_to_send(&caller, args.community_rules_accepted, state)?;
