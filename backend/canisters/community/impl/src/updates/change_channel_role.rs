...
fn change_channel_role(args: Args) -> Response {
    match execute_update(|state| change_channel_role_impl(args, state)) {
        Ok(errors) => {
            if errors.is_empty() {
                Response::Success
            } else {
...
fn change_channel_role_impl(mut args: Args, state: &mut RuntimeState) -> OCResult<HashMap<UserId, OCError>> {
    ...
    let results = channel.chat.change_role(member.user_id, args.user_ids, args.new_role, now);
