...
async fn c2c_join_channel(args: Args) -> Response {
    ...
    .await
    {
        community_canister::c2c_join_community::Response::Success(_) => {
            let response = join_channel_impl(args.channel_id, args.principal, false).await;
        ...
        community_canister::c2c_join_community::Response::AlreadyInCommunity(_) => {
            join_channel_impl(args.channel_id, args.principal, false).await
        }
...
pub(crate) async fn join_channel_impl(channel_id: ChannelId, user_principal: Principal, auto_join: bool) -> Response {
    match read_state(|state| is_permitted_to_join(channel_id, user_principal, state)) {
        Ok(Some((gate, user_index_canister_id, user_id))) => {
            if auto_join {
                return GateCheckFailed(types::GateCheckFailedReason::CannotAutoJoin);
            }
