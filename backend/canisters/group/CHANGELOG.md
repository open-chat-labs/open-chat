...
- Reject every update call while the group is frozen in one place, `execute_update`, by trapping, rather than per endpoint with a `ChatFrozen` error. Only the endpoints which must keep working while frozen run via `execute_update_even_if_frozen`: unfreezing, platform moderation, exporting into a community, `c2c_local_index`, P2P swap status notifications, ending video calls and `wallet_receive`. Among others, `c2c_update_proposals`, `c2c_uninstall_bot`, `decline_invitation` and `toggle_mute_notifications`, which previously worked while frozen, are now blocked ([#9533](https://github.com/open-chat-labs/open-chat/pull/9533))
...
## [[2.0.1950](https://github.com/open-chat-labs/open-chat/releases/tag/v2.0.1950-group)] - 2026-01-14
