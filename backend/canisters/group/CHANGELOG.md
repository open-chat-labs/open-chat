...
- Reject every update call while the group is frozen in one place, `execute_update`, by trapping, rather than per endpoint with a `ChatFrozen` error. Only freezing, unfreezing, exporting the group into a community (which happens while it is frozen) and `wallet_receive` run via `execute_update_even_if_frozen`. Every other update, including those which previously worked while frozen such as platform moderation, `c2c_local_index` and ending video calls, is now blocked ([#9533](https://github.com/open-chat-labs/open-chat/pull/9533))
