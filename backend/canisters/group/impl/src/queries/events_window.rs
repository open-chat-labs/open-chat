...
fn c2c_events_window(args: C2CArgs) -> Response {
    read_state(|state| events_window_impl(args.args, Some(args.caller), args.bot_api_key_secret, state))
}
...
fn events_window_impl(
    ...
    let Some(events_caller) = state.data.get_caller_for_events(caller, bot_api_key_secret) else {
