...
async fn change_role(args: Args) -> Response {
    ...
    if matches!(args.new_role, Role::Owner) {
...
fn change_role_impl(args: Args, runtime_state: &mut RuntimeState) -> Response {
    ...
    let event = match runtime_state.data.participants.change_role(
        caller,
        &args.user_id,
        args.new_role,
        &runtime_state.data.permissions,
        true,
    ) {
