...
fn process_event<F: FnOnce() -> TimestampMillis>(
    ...
    match event {
        ...
        LocalIndexEvent::MessageClassified(ev) => {
            ...
            if let Some(categories) = ModerationCategories::from_bits(ev.flags).map(|c| c | sticky)
                && {
                    let flag_result =
                        state
                            .data
        ...
        LocalIndexEvent::MediaScanMatched(ev) => {
            ...
            if let Some((message_index, sender, content_excerpt, mut blob_references, existing_flags)) = state
                .data
                .chat
                .events
                .message_internal(EventIndex::default(), ev.thread_root_message_index, ev.message_id.into())
        ...
        LocalIndexEvent::UserIdMigrated(ev) => {
            state.data.change_user_id(ev.old_user_id, ev.new_user_id, ev.principal, **now);
        }
    }
