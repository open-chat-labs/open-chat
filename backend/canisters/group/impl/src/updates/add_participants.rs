...
async fn add_participants(args: Args) -> Response {
    ...
    if !users_added.is_empty() {
        ...
        mutate_state(|state| commit(prepare_result.added_by, added_by_name, &users_added, args.correlation_id, state));
    }
...
fn commit(added_by: UserId, added_by_name: String, users: &[(UserId, Principal)], correlation_id: u64, runtime_state: &mut RuntimeState) {
