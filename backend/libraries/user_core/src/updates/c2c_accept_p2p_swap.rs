...
// recorded. The caller then deposits token1 into the escrow canister from the user's account or
// the one they approved.
pub fn prepare(user: &mut User, my_user_id: UserId, args: &mut Args, now: TimestampMillis) -> OCResult {
    user.pin_number.verify(args.pin.as_mut(), now)?;
    ledger_utils::validate_from_account(args.from_account, my_user_id.canister_id())?;
    Ok(())
}

// Records the swap against the user, its deposit having been made
