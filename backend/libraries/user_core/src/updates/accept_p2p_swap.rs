...
use types::{Chat, MessageId, OCResult, P2PSwapAccepted, P2PSwapLocation, ReserveP2PSwapSuccess, TimestampMillis, UserId};
...
// Checks the acceptance and reserves the swap for the user. The caller then deposits token1 into
// the escrow canister from the user's account or the one they approved.
pub fn prepare(user: &mut User, my_user_id: UserId, args: &mut Args, now: TimestampMillis) -> OCResult<Reserved> {
