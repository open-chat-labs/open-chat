...
pub const B: Cycles = 1_000_000_000;
pub const T: Cycles = 1_000 * B;
...
const MIN_CYCLES_BALANCE: Cycles = 50 * T; // 50T
...
pub const ONE_SEC_MINTER_CANISTER_ID: CanisterId = Principal::from_slice(&[0, 0, 0, 0, 2, 48, 11, 124, 1, 1]);

// wji62-oiaaa-aaaaf-bsc7a-cai. Local deployments and the integration tests
// create the canister with this exact id (specified-id creation), so the
// same constant is correct in every environment.
pub const PERSONHOOD_VERIFIER_CANISTER_ID: CanisterId = Principal::from_slice(&[0, 0, 0, 0, 0, 161, 144, 190, 1, 1]);
...
mod tests {
    ...
    #[test]
    fn personhood_verifier_canister_id() {
        assert_eq!(
            PERSONHOOD_VERIFIER_CANISTER_ID,
            Principal::from_text("wji62-oiaaa-aaaaf-bsc7a-cai").unwrap()
        );
    }
