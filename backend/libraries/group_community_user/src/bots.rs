...
impl InstalledBots {
    ...
    pub fn iter_latest_updates(
        &self,
        since: TimestampMillis,
    ) -> impl Iterator<Item = (UserId, BotUpdate)> + '_ {
        self.updates
            .iter()
            .rev()
    ...
    fn prune_then_insert_member_update(
        &mut self,
        user_id: UserId,
        update: BotUpdate,
        now: TimestampMillis,
...
impl BotApiKeys {
    ...
    pub fn permissions_if_secret_matches(
        &self,
        bot_id: &UserId,
        secret: &str,
