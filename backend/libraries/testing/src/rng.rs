...
pub fn random_principal() -> Principal {
    deterministic::random_principal(&mut rng())
}

pub fn random_from_principal<T: From<Principal>>() -> T {
    deterministic::random_from_principal(&mut rng())
}

pub fn random_internet_identity_principal() -> (Principal, Vec<u8>) {
    deterministic::random_internet_identity_principal(&mut rng())
}

pub fn random_delegated_principal(originating_canister_id: CanisterId) -> (Principal, Vec<u8>) {
    deterministic::random_delegated_principal(&mut rng(), originating_canister_id)
}

pub fn random_string() -> String {
    deterministic::random_string(&mut rng())
}

pub fn random_from_u32<T: From<u32>>() -> T {
    deterministic::random_from_u32(&mut rng())
}

pub fn random_from_u128<T: From<u128>>() -> T {
    deterministic::random_from_u128(&mut rng())
}

pub mod deterministic {
    use super::*;

    pub fn random_principal<R: Rng>(rng: &mut R) -> Principal {
        random_from_principal(rng)
    }

    pub fn random_from_principal<R: Rng, T: From<Principal>>(rng: &mut R) -> T {
        let random_bytes = rng.next_u64().to_ne_bytes();

        Principal::from_slice(&random_bytes).into()
    }

    pub fn random_internet_identity_principal<R: Rng>(rng: &mut R) -> (Principal, Vec<u8>) {
        random_delegated_principal(rng, NNS_INTERNET_IDENTITY_CANISTER_ID)
    }

    pub fn random_delegated_principal<R: Rng>(rng: &mut R, originating_canister_id: CanisterId) -> (Principal, Vec<u8>) {
    ...
    pub fn random_string<R: Rng>(rng: &mut R) -> String {
        rng.next_u32().to_string()
    }

    pub fn random_from_u32<R: Rng, T: From<u32>>(rng: &mut R) -> T {
        let value: u32 = rng.next_u32();
        value.into()
    }

    pub fn random_from_u128<R: Rng, T: From<u128>>(rng: &mut R) -> T {
