...
import { currentChatDraftMessage, updateSummaryWithConfirmedMessage, chatStateStore, addServerEventsToStores, confirmedEventIndexesLoaded, clearServerEvents, } from "../../stores/chat";
import { userStore } from "../../stores/user";
import { rollbar } from "../../utils/logging";
import { toastStore } from "../../stores/toast";
import { localMessageUpdates } from "../../stores/localMessageUpdates";
import { indexRangeForChat, makeRtcConnections, mergeSendMessageResponse, serialiseMessageForRtc, upToDate, userIdsFromEvents, } from "../../domain/chat/chat.utils";
...
export function selectReaction(api, chat, userId, threadRootMessageIndex, messageId, reaction, username, kind) {
...
export function deleteMessage(api, chat, userId, threadRootMessageIndex, messageId) {
...
export async function updateUserStore(api, chatId, userId, userIdsFromEvents) {
    ...
    const resp = await api.getUsers({
        userGroups: [
            {
                users: missingUserIds(get(userStore), new Set(allUserIds)),
...
export async function editMessage(api, chat, msg, threadRootMessageIndex) {
...
export function registerPollVote(api, userId, chatId, threadRootMessageIndex, messageId, messageIndex, answerIndex, type) {
...
function blockUserLocally(chatId, userId) {
...
function unblockUserLocally(chatId, userId) {
    ...
    chatStateStore.updateProp(chatId, "members", (p) => {
        ...
        return [
            ...p,
            {
                role: "participant",
                userId,
                username: (_b = (_a = get(userStore)[userId]) === null || _a === void 0 ? void 0 : _a.username) !== null && _b !== void 0 ? _b : "unknown",
            },
        ];
    });
}
export function blockUser(api, chatId, userId) {
...
export async function loadEventWindow(api, user, serverChat, chat, messageIndex) {
    if (messageIndex >= 0) {
        const range = indexRangeForChat(serverChat);
        const eventsPromise = chat.kind === "direct_chat"
            ? api.directChatEventsWindow(range, chat.them, messageIndex, chat.latestEventIndex)
            : api.groupChatEventsWindow(range, chat.chatId, messageIndex, chat.latestEventIndex);
...
export async function handleEventsResponse(api, user, chat, resp, keepCurrentEvents = true) {
    if (resp === "events_failed")
        return;
    if (!keepCurrentEvents) {
        clearServerEvents(chat.chatId);
        chatStateStore.setProp(chat.chatId, "userGroupKeys", new Set());
    }
    ...
    const events = resp.events.concat(resp.affectedEvents.filter((e) => indexIsInRanges(e.index, confirmedLoaded)));
...
function isContiguous(chatId, response) {
    const confirmedLoaded = confirmedEventIndexesLoaded(chatId);
    if (confirmedLoaded.length === 0 || response.events.length === 0)
        return true;
    ...
    if (!isContiguous) {
        console.log("Events in response are not contiguous with the loaded events", confirmedLoaded, firstIndex, lastIndex);
    }
    return isContiguous;
}
function loadEvents(api, serverChat, clientChat, startIndex, ascending) {
    return api.chatEvents(clientChat, indexRangeForChat(serverChat), startIndex, ascending, undefined, clientChat.latestEventIndex);
}
export async function loadPreviousMessages(api, user, serverChat, clientChat) {
...
function latestServerEventIndex(serverChat) {
    return serverChat.latestEventIndex;
}
function earliestAvailableEventIndex(clientChat) {
    return clientChat.kind === "group_chat" ? clientChat.minVisibleEventIndex : 0;
}
function previousMessagesCriteria(serverChat, clientChat) {
...
function earliestLoadedIndex(chatId) {
    const confirmedLoaded = confirmedEventIndexesLoaded(chatId);
    return confirmedLoaded.length > 0 ? confirmedLoaded.index(0) : undefined;
}
export async function loadNewMessages(api, user, serverChat, clientChat) {
    ...
    const newLatestMessage = latestMessage !== undefined && latestMessage.index > latestServerEventIndex(serverChat);
    if (newLatestMessage) {
        updateSummaryWithConfirmedMessage(clientChat.chatId, latestMessage);
    }
    return newLatestMessage;
}
function newMessageCriteria(serverChat) {
...
function confirmedUpToEventIndex(chatId) {
...
export function morePreviousMessagesAvailable(clientChat) {
    var _a;
    return (((_a = earliestLoadedIndex(clientChat.chatId)) !== null && _a !== void 0 ? _a : Number.MAX_VALUE) >
        earliestAvailableEventIndex(clientChat));
}
export function moreNewMessagesAvailable(serverChat) {
    var _a;
    return ((_a = confirmedUpToEventIndex(serverChat.chatId)) !== null && _a !== void 0 ? _a : -1) < latestServerEventIndex(serverChat);
}
export function refreshAffectedEvents(api, user, clientChat, affectedEventIndexes) {
    ...
    const eventsPromise = clientChat.kind === "direct_chat"
        ? api.directChatEventsByEventIndex(clientChat.them, filtered, undefined, clientChat.latestEventIndex)
        : api.groupChatEventsByEventIndex(clientChat.chatId, filtered, undefined, clientChat.latestEventIndex);
    return eventsPromise.then((resp) => handleEventsResponse(api, user, clientChat, resp));
}
export async function loadDetails(api, user, clientChat, currentEvents) {
    ...
    if (clientChat.kind === "group_chat") {
        if (!chatStateStore.getProp(clientChat.chatId, "detailsLoaded")) {
            ...
            if (resp !== "caller_not_in_group") {
                ...
                chatStateStore.setProp(clientChat.chatId, "latestEventIndex", resp.latestEventIndex);
            ...
            await updateUserStore(api, clientChat.chatId, user.userId, userIdsFromEvents(currentEvents));
        }
        else {
...
export async function updateDetails(api, user, clientChat, currentEvents) {
    if (clientChat.kind === "group_chat") {
...
function addPinnedMessage(chatId, messageIndex) {
    chatStateStore.updateProp(chatId, "pinnedMessages", (s) => {
        s.add(messageIndex);
        return new Set(s);
    });
}
function removePinnedMessage(chatId, messageIndex) {
    chatStateStore.updateProp(chatId, "pinnedMessages", (s) => {
        s.delete(messageIndex);
        return new Set(s);
    });
}
export function unpinMessage(api, clientChat, messageIndex) {
    if (clientChat.kind === "group_chat") {
...
export function pinMessage(api, clientChat, messageIndex) {
    if (clientChat.kind === "group_chat") {
        addPinnedMessage(clientChat.chatId, messageIndex);
...
export function removeMessage(currentUserId, clientChat, messageId, userId) {
...
export async function sendMessage(api, user, serverChat, clientChat, currentEvents, messageEvent) {
    let jumpingTo = undefined;
    if (!upToDate(clientChat, currentEvents)) {
        jumpingTo = await loadEventWindow(api, user, serverChat, clientChat, 
    ...
    messagesRead.markMessageRead(clientChat.chatId, messageEvent.event.messageIndex, messageEvent.event.messageId);
    currentChatDraftMessage.clear(clientChat.chatId);
    return jumpingTo;
}
export async function handleMessageSentByOther(api, user, clientChat, currentEvents, messageEvent) {
    const confirmedLoaded = confirmedEventIndexesLoaded(clientChat.chatId);
    if (indexIsInRanges(messageEvent.index, confirmedLoaded)) {
        // We already have this confirmed message
        return;
    }
    const isAdjacentToAlreadyLoadedEvents = indexIsInRanges(messageEvent.index - 1, confirmedLoaded) ||
...
export function forwardMessage(api, user, serverChat, clientChat, currentEvents, evt) {
...
export function sendMessageWithAttachment(api, user, serverChat, clientChat, currentEvents, evt, mentioned) {
...
function onSendMessageSuccess(chatId, resp, msg) {
