...
type WebModelState = {
    ...
    name?: string;
    status: "none" | "attached" | "loading" | "loaded" | "error";
    error?: string;
};

const state: WebModelState = { status: "none" };

/** UI-facing snapshot: the attached model's file name + lifecycle status. */
export const webModelStatus = writable<{ name?: string; status: WebModelState["status"]; error?: string }>({
    status: "none",
});

function publish(): void {
    webModelStatus.set({ name: state.name, status: state.status, error: state.error });
}
...
/** Drop the attached model, free the wasm runtime, and forget the persisted handle. */
...
async function ensureLoaded(): Promise<void> {
    ...
    if (state.file === undefined) throw new Error("no browser model attached");
    state.status = "loading";
    publish();
    try {
        const { Wllama } = await import("@wllama/wllama");
        runtime = new Wllama({ default: wllamaWasm }, { suppressNativeLog: true });
        await runtime.loadModel([state.file], {
            n_ctx: 4096,
