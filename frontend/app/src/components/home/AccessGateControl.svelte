<script lang="ts">
    ...
    $: candidateTokenDetails = client.getTokenDetailsForSnsAccessGate(candidate.gate);
    ...
    onMount(() => {
        ...
        paymentGateBindings = getPaymentGateBindings($cryptoLookup);
        ...
        selectedNeuronGateKey = gateBindings.find(
            (g) =>
                candidate.gate.kind === "neuron_gate" && candidate.gate.governanceCanister === g.key
        )?.key;

        selectedPaymentGateKey = gateBindings.find(
            (g) => candidate.gate.kind === "payment_gate" && candidate.gate.ledgerCanister === g.key
        )?.key;
    });
