<script lang="ts">
    ...
    import { MAX_RULES_LENGTH, updateGroupState } from "./group.svelte";
    import GroupCard from "./GroupCard.svelte";

    let ugs = updateGroupState;
</script>

<SlidingPageContent title={i18nKey("Rules", undefined, ugs.candidateGroup.level, true)}>
    <Container
        ...
        <Container padding={["zero", "md"]} gap={"xl"} direction={"vertical"}>
            <Setting
                toggle={() =>
                    (ugs.candidateGroup.rules.enabled = !ugs.candidateGroup.rules.enabled)}
                ...
                <Switch bind:checked={ugs.candidateGroup.rules.enabled} />
            </Setting>
            {#if ugs.candidateGroup.rules.enabled}
                <Setting
                    toggle={() =>
                        (ugs.candidateGroup.rules.newVersion =
                            !ugs.candidateGroup.rules.newVersion)}
                    info={"rules.promptExistingUsersInstructions"}
                    title={"rules.promptExistingUsers"}>
                    <Switch bind:checked={ugs.candidateGroup.rules.newVersion} />
                </Setting>
            {/if}
        </Container>
        ...
        {#if ugs.candidateGroup.rules.enabled}
            <TextArea
                bind:value={ugs.candidateGroup.rules.text}
                minlength={0}
                maxlength={MAX_RULES_LENGTH}
                rows={8}
                error={!ugs.rulesValid}
                placeholder={interpolate(
                    $_,
                    i18nKey("rules.placeholder", undefined, ugs.candidateGroup.level, true),
                )}>
                {#snippet subtext()}
                    <Translatable
                        resourceKey={i18nKey(
