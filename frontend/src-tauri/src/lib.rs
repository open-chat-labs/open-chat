...
pub fn run() {
    ...
    // CrabNebula DevTools telemetry — opt-in at build time: the `devtools`
    // Cargo feature plus OC_DEVTOOLS=1. The development launcher supplies the
    // feature when this environment flag is set; release builds do not.
    //
    ...
    // memory during the crash flowed through tauri_plugin_devtools. The optional
    // dependency is excluded from ordinary builds and the collector can only run
    // in debug builds, so a plain reload no longer starts it accidentally.
    //
    ...
    // rebuild with `--features devtools` and OC_DEVTOOLS=1 when you actually need
    // the inspector (or use the development launcher with OC_DEVTOOLS=1).
