export const idlFactory = ({ IDL }) => {
    ...
    const VaultBucketsResponse = IDL.Variant({
        Success: IDL.Record({ buckets: IDL.Vec(IDL.Principal) }),
    });
    ...
    return IDL.Service({
        allocated_bucket_v2: IDL.Func([AllocatedBucketArgs], [AllocatedBucketResponse], ["query"]),
        can_forward: IDL.Func([CanForwardArgs], [CanForwardResponse], ["query"]),
        user: IDL.Func([UserArgs], [UserResponse], ["query"]),
        vault_buckets: IDL.Func([IDL.Record({})], [VaultBucketsResponse], ["query"]),
    });
};
